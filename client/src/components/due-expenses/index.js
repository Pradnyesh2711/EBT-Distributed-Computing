...
const DueExpenses = () => {
  ...
  useEffect(() => {
    axios.get("http://172.16.30.78:5000/api/budget").then((res) => {
  ...
  const fetchExpenses = async () => {
    try {
      let tempLimit = limit;
      const response = await axios.get("http://172.16.30.78:5000/api/expenses");
  ...
  const handleLimit = async () => {
    try {
      ...
      const existingBudgetRes = await axios.get(
        "http://172.16.30.78:5000/api/budget"
      );
      console.log(existingBudgetRes.data);
      if (existingBudgetRes.data !== null) {
        ...
        if (existingBudget) {
          ...
          const updateRes = await axios.put(
            `http://172.16.30.78:5000/api/budget/${budgetId}`,
            {
              amount: limit,
            }
          );
        ...
        const res = await axios.post("http://172.16.30.78:5000/api/budget", {
          amount: limit,
        });
        if (res.status >= 200 && res.status <= 300) {
